...
{
    public class BrainFlowSingleChannelDataStreamVisualizer : MonoBehaviour
    {
        
        public VisualizationType visualizationType;
        public BrainFlowSingleChannelDataStream channelDataStream;
        ...
        private BrainFlowChannelTypeDashboard channelTypeDashboard;
        ...
        public void Initialize(BrainFlowSingleChannelDataStream dataStream, string dataType)
        {
            channelDataStream = dataStream;
            channelID = channelDataStream.channelID;
            brainFlowSessionProfile = channelDataStream.brainFlowSessionProfile;
            dType = dataType;
            channelTypeDashboard = brainFlowSessionProfile.DataFlowBoards[dType];
            ...
            yHeight = dashBoardRect.sizeDelta.y / 16;
        ...
        private void Update()
        {
            ...
            numberOfDataPoints = Mathf.RoundToInt(channelTypeDashboard.xRange);
            xInterval = graphContainerRect.sizeDelta.x / (numberOfDataPoints+1);
            var yAnchor = (graphContainerRect.sizeDelta.y / (channelDataStream.numberOfChannels+1)) * channelDataStream.channelTypeID;
            rect.anchoredPosition = new Vector2(0, yAnchor);
            if(dataObjectTotal < numberOfDataPoints) CreateGraphObjects();
            if (channelDataStream.channelData.Count <= numberOfDataPoints)
            {
                graphData = channelDataStream.channelData;
                return;
            }
            
            graphData = channelDataStream.channelData.GetRange(
                channelDataStream.channelData.Count - 1 - numberOfDataPoints, 
                numberOfDataPoints);
        }
        ...
        private void CreateGraphObjects()
        {
            while (dataObjectTotal < numberOfDataPoints)
            {
                ...
                newDataBar.AddComponent<BrainFlowChannelDataPointController>().CreateBar(this, dataObjectTotal);
                dataObjectTotal++;
            }
        }
